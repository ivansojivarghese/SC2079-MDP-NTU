...
//        pathTranslator.translatePath("MOVE,FORWARD,30");
    ...
    public static void printMessage(String target, String message) {
        showLog("Entering printMessage");
        editor = sharedPreferences.edit();
        String targetMsg = target + "#" + message;

        Log.d("DEBUG", targetMsg);
        if (BluetoothConnectionService.BluetoothConnectionStatus) {
            byte[] bytes = targetMsg.getBytes(StandardCharsets.UTF_8);
            BluetoothConnectionService.write(bytes);
        }
        showLog(message);
...
//        newDir = newDir.toUpperCase();
        ...
        if ((x - 2)>=0 && (y - 1)>=0)
        {
//          BluetoothCommunications.getMessageReceivedTextView().append("ROBOT" + "," + (col - 2)*10 + "," + (row - 1)*10 + "," + dir.toUpperCase());
            // Update robot new direction
            ...
            gridMap.obstacles.put(-1,  "[" + (x-2)*10 + "," + (y-1)*10 + "," + dir.toUpperCase() + "]");
            Home.printMessage(Target.RPI, "ROBOT" + "," + (x-2)*10 + "," + (y-1)*10 + "," + dir.toUpperCase());
        }
        else{
            showLog("out of grid");
        }
...
//        BluetoothCommunications.getMessageReceivedTextView().append("ROBOT,"+ (x-1) +"," + (y-1) + "," + dir+"\n"); //for troubleshooting
    ...
    public static void refreshLabel() {
        xAxisTextView.setText(String.valueOf(gridMap.getCurCoord()[0]-2));
        yAxisTextView.setText(String.valueOf(gridMap.getCurCoord()[1]-1));
        // xAxisTextView.setText(String.valueOf(gridMap.getCurCoord()[0]-2) + " (" + ((gridMap.getCurCoord()[0] - 2) * 10) + ")");
        // yAxisTextView.setText(String.valueOf(gridMap.getCurCoord()[1]-1) + " (" + ((gridMap.getCurCoord()[1] - 1) * 10) + ")");
...
//            showLog("cmd2 --- " + cmdd[2]);
            ...
            if (message.contains("STATUS")) {
                String rawStmCmd = message.split("#")[1];
                Log.d(TAG, "rawStmCmd: " + rawStmCmd);
                robotStatusTextView.setText(rawStmCmd);
                pathTranslator.translatePath(rawStmCmd);
            }
            //ROBOT|5,4,EAST (Early version of updating robot position via comms)
                     if(message.contains("ROBOT")) {
                String[] sentCoords = message.split(",");
                // String[] sentDirection = sentCoords[2].split("\\.");
                String sentDirection = sentCoords[3];
                String direction = "";
...
//                BluetoothCommunications.getMessageReceivedTextView().append("\n");
                ...
                gridMap.setCurCoord(((Integer.valueOf(sentCoords[1]) / 10) + 2), 1 + (Integer.valueOf(sentCoords[2]) / 10), direction);
...
//            else if(message.contains("MOVE") || message.contains("TURN") || message.contains("FW") || message.contains("BW") || message.contains("FC") || message.contains("BC") || message.contains("FA") || message.contains("BA")){
